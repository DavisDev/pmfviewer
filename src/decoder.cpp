...
int T_Decoder(SceSize _args, void *_argp)
{
	...
	for(;;)
	{
		...
		if( iThreadsRunning == 0 &&
			IsRingbufferFull(D->Reader)
			 && D->Video->m_iNumBuffers == D->Video->m_iFullBuffers// && D->Audio->m_iNumBuffers == D->Audio->m_iFullBuffers 
			)
		{
			iThreadsRunning = 1;
		...
		if(D->Audio->m_iFullBuffers < D->Audio->m_iNumBuffers)
		{
			...
			else
			{
				...
				if(D->m_MpegAuAtrac->iPts & 0X80000000)
		...
		if(D->Video->m_iFullBuffers < D->Video->m_iNumBuffers)
		{
			...
			else
			{
				...
				retVal = sceMpegAvcDecode(&D->m_Mpeg, D->m_MpegAuAVC, D->Video->m_iTexWidth, &D->Video->m_pVideoBuffer[D->Video->m_iPlayBuffer], &iVideoStatus);
				if(retVal != 0)
				{
				...
				if(D->m_MpegAuAVC->iPts & 0x80000000)
	...
	sceMpegAvcDecodeStop(&D->m_Mpeg, D->Video->m_iTexWidth, D->Video->m_pVideoBuffer, &iVideoStatus);
