...
int T_Audio(SceSize _args, void *_argp)
{
	...
	for(;;)
	{
		...
		{			
			...
			if (D->Audio->m_speed == 2){
				for (int i = 1; i < D->Audio->m_AudioSamples; i++)
					((int*)D->Audio->m_pAudioBuffer[D->Audio->m_iPlayBuffer])[i] = 
					((int*)D->Audio->m_pAudioBuffer[D->Audio->m_iPlayBuffer])[i*2];
			}
...
{
	int i = 0, fail = 0;
//	Audio.m_AudioChannel = sceAudioChReserve(-1, m_MpegAtracOutSize/4, PSP_AUDIO_FORMAT_STEREO);
	Audio.m_AudioChannel = sceAudioSRCChReserve(m_MpegAtracOutSize/4, 44100, 2);
	if(Audio.m_AudioChannel < 0)
	{
		sprintf(m_LastError, "sceAudioSRCChReserve() failed: 0x%08X", (int)Audio.m_AudioChannel);
		return -1;
	}
